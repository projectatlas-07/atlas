...
declare
  ...
  conflicting_week_start date;
begin
  select weekly_earnings.factory_id, weekly_earnings.week_start
    into conflicting_factory_id, conflicting_week_start
    from public.weekly_earnings
    where weekly_earnings.labour_group_id is not null
    group by weekly_earnings.factory_id, weekly_earnings.week_start
    having count(*) > 1
    limit 1;

  if conflicting_factory_id is not null then
    raise exception 'Cannot enforce one mud earning per factory/week: factory % has multiple group earnings for week %.', conflicting_factory_id, conflicting_week_start
      using errcode = 'P0001';
  end if;
end;
$$;

create unique index weekly_earnings_one_group_per_factory_week_idx
  on public.weekly_earnings (factory_id, week_start)
