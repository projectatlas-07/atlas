create table public.staff_salary_deductions (
  ...
  constraint staff_salary_deductions_id_factory_key unique (id, factory_id),
  constraint staff_salary_deductions_date_check check (isfinite(deduction_date)),
  constraint staff_salary_deductions_amount_check
    check (amount > 0 and amount <> 'NaN'::numeric),
  constraint staff_salary_deductions_reason_check
    check (reason is null or (reason = btrim(reason) and reason <> '')),
  constraint staff_salary_deductions_worker_factory_fkey
    foreign key (staff_worker_id, factory_id)
    references public.staff_workers (id, factory_id) on delete restrict
);

create index staff_salary_deductions_worker_history_idx
  on public.staff_salary_deductions (
    factory_id,
    staff_worker_id,
    deduction_date desc,
    created_at desc,
...
create or replace function public.prevent_staff_salary_deduction_mutation()
returns trigger
language plpgsql
set search_path = pg_catalog, public
as $$
begin
  raise exception 'Staff salary deductions are immutable.' using errcode = 'P2530';
end;
$$;

create trigger staff_salary_deductions_are_immutable
before update or delete on public.staff_salary_deductions
for each row execute function public.prevent_staff_salary_deduction_mutation();

alter table public.staff_salary_deductions enable row level security;

revoke all on public.staff_salary_deductions from anon;
revoke all on public.staff_salary_deductions from authenticated;
grant select on public.staff_salary_deductions to authenticated;

create policy "Authenticated users can read their factory Staff salary deductions"
  on public.staff_salary_deductions for select to authenticated
  using (exists (
    select 1 from public.factory_users
    where factory_users.user_id = auth.uid()
      and factory_users.factory_id = staff_salary_deductions.factory_id
...
create function public.get_staff_financial_summary(
  p_factory_id uuid,
...
begin
  ...
  select coalesce(sum(staff_salary_deductions.amount), 0)
  into total_deductions
  from public.staff_salary_deductions
  where staff_salary_deductions.factory_id = p_factory_id
    and staff_salary_deductions.staff_worker_id = p_staff_worker_id
    and staff_salary_deductions.deduction_date <= business_today;
  ...
  available_balance := total_earnings - total_deductions - total_withdrawn;
...
create function public.create_staff_withdrawal(
  p_factory_id uuid,
  p_staff_worker_id uuid,
...
create or replace function public.create_staff_salary_deduction(
  p_factory_id uuid,
  p_staff_worker_id uuid,
...
returns table (
  deduction_id uuid,
  deduction_factory_id uuid,
  deduction_staff_worker_id uuid,
  deduction_date date,
...
declare
  ...
  summary record;
  new_deduction public.staff_salary_deductions%rowtype;
begin
  if auth.uid() is null or not exists (
    select 1 from public.factory_users
    where factory_users.user_id = auth.uid()
  ...
  if p_deduction_date is null or not isfinite(p_deduction_date) then
    raise exception 'deduction_date must be a valid finite date.'
      using errcode = '22023';
  end if;
  if p_deduction_date > business_today then
    raise exception 'deduction_date cannot be later than the current business date (%).',
      business_today using errcode = '22023';
  end if;
  ...
  if p_amount > summary.available_balance then
    raise exception 'Deduction amount % exceeds available Staff balance %.',
      p_amount, summary.available_balance using errcode = 'P0001';
  end if;
  ...
  insert into public.staff_salary_deductions (
    ...
    p_factory_id, p_staff_worker_id, p_deduction_date, p_amount, normalized_reason
  ) returning * into new_deduction;

  deduction_id := new_deduction.id;
  deduction_factory_id := new_deduction.factory_id;
  deduction_staff_worker_id := new_deduction.staff_worker_id;
  deduction_date := new_deduction.deduction_date;
  deduction_amount := new_deduction.amount;
  deduction_reason := new_deduction.reason;
  created_at := new_deduction.created_at;
  total_earnings := summary.total_earnings;
  total_deductions := summary.total_deductions + new_deduction.amount;
  total_withdrawn := summary.total_withdrawn;
  available_balance := summary.available_balance - new_deduction.amount;
  return next;
end;
$$;

revoke all on function public.prevent_staff_salary_deduction_mutation() from public;
...
revoke all on function public.create_staff_salary_deduction(uuid, uuid, date, numeric, text) from public;
revoke all on function public.create_staff_salary_deduction(uuid, uuid, date, numeric, text) from anon;
grant execute on function public.create_staff_salary_deduction(uuid, uuid, date, numeric, text) to authenticated;
