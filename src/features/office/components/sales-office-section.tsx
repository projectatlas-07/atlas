...
export function SalesOfficeSection({
  ...
  function cacheCreatedCustomer(customer: Customer) {
  ...
  function handleSaved(saved: Challan, action: "created" | "updated") {
    ...
    setSuccess(
      action === "created"
        ? `Challan #${saved.challanNumber} created. Authoritative total: ${formatSalesMoney(saved.challanTotal)}.`
        : `Challan #${saved.challanNumber} updated. Authoritative total: ${formatSalesMoney(saved.challanTotal)}.`,
    );
  }
  ...
  async function confirmVoid(challan: Challan) {
    ...
    try {
      ...
      cacheSavedChallan(saved);
      setIsConfirmingVoid(false);
      setSuccess(`Challan #${saved.challanNumber} is now Void. Its permanent number is preserved.`);
    } catch (error) {
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-600">
            Each successful save receives its permanent number and database-calculated total.
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(22rem,0.8fr)] xl:items-start">
        <div>
          {mode === "create" && <ChallanEditor
            ...
            onCustomerCreated={cacheCreatedCustomer}
            ...
            {challans.map((challan) => <li key={challan.id} className={selectedChallanId === challan.id ? "bg-cyan-50" : "bg-white"}>
              <button type="button" onClick={() => openChallan(challan.id)} className="w-full px-5 py-4 text-left hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-cyan-600">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-bold">Challan #{challan.challanNumber}</p>
...
function ChallanEditor({
  ...
  const normalizedVehicleSearch = vehicleSearch.trim().replace(/\s+/g, "").toUpperCase();
  const selectableVehicles = activeVehicles.filter((vehicle) =>
    vehicle.id === form.vehicleId
    || !normalizedVehicleSearch
    || vehicle.normalizedVehicleNumber.includes(normalizedVehicleSearch),
  );
  const totalPreview = calculateChallanTotalPreview(form.lines, form.flexibleLines);

  function updateLine(key: string, field: "brickTypeId" | "quantity" | "ratePer1000Bricks", value: string) {
    setForm((current) => ({
      ...current,
  ...
  return (
    <section aria-labelledby="challan-editor-heading" className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 id="challan-editor-heading" className="text-xl font-bold">{challan ? `Edit Challan #${challan.challanNumber}` : "Create Challan"}</h3>
          {!challan && <p className="mt-1 text-sm text-slate-500">The permanent number appears only after a successful save.</p>}
        </div>
      </div>
      ...
      <form className="mt-6" onSubmit={(event) => void submitChallan(event)}>
        ...
        <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
          <Field label="Customer">
            <select
              ...
              onChange={(event) => { setForm((current) => ({ ...current, customerId: event.target.value })); setError(""); }}
              disabled={customersUnavailable || isSaving || customers.length === 0}
          ...
          <button type="button" onClick={() => { setShowQuickCustomer((current) => !current); setCustomerError(""); }} disabled={isSaving} className={secondaryButton}>
            {showQuickCustomer ? "Close customer form" : "Quick add customer"}
          </button>
        </div>

        {selectedCustomer && <div className="mt-3 grid gap-1 rounded-lg border border-cyan-100 bg-cyan-50 px-4 py-3 text-sm sm:grid-cols-2">
          <p className="font-semibold text-slate-900">{selectedCustomer.name}</p>
          <p className="text-slate-700 sm:text-right">{selectedCustomer.mobile || "No mobile"}</p>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <Field label="Search Vehicles"><input type="search" value={vehicleSearch} onChange={(event) => setVehicleSearch(event.target.value)} placeholder="WB12AB1234" autoCapitalize="characters" disabled={isSaving || vehiclesUnavailable} className={inputClass} /></Field>
          <Field label="Vehicle (optional)">
            <select
              value={form.vehicleId}
              onChange={(event) => { setForm((current) => selectVehicleForChallan(current, vehicles, event.target.value)); setError(""); }}
              disabled={isSaving || vehiclesUnavailable}
              className={inputClass}
            >
              <option value="">No vehicle</option>
              {selectedVehicle && !selectedVehicle.isActive && <option value={selectedVehicle.id}>{selectedVehicle.vehicleNumber} · archived</option>}
              {selectableVehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.vehicleNumber} · Wage tracking {vehicle.deliveryWageTrackingEnabled ? "ON" : "OFF"}</option>)}
            </select>
          </Field>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          ...
          {selectedVehicle && <p className="text-sm text-slate-600">Selected: <span className="font-semibold text-slate-900">{selectedVehicle.vehicleNumber}</span> · Delivery Wage Tracking {selectedVehicle.deliveryWageTrackingEnabled ? "ON" : "OFF"}</p>}
        </div>
        ...
        {form.vehicleDeliveryWageTrackingEnabled && <div className="mt-4 max-w-sm rounded-lg border border-amber-200 bg-amber-50 p-4">
          <Field label="Trip Labour Wage"><input type="number" min="0.01" step="0.01" value={form.tripLabourWage} onChange={(event) => { setForm((current) => ({ ...current, tripLabourWage: event.target.value })); setError(""); }} placeholder="750" disabled={isSaving} className={inputClass} /></Field>
        ...
        <div className="mt-6 border-t border-slate-200 pt-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">Rate is ₹ per 1,000 bricks.</p>
            </div>
            <button
              type="button"
              onClick={() => {
          ...
          <div className="mt-4 space-y-3">
            {form.lines.map((line, index) => {
              ...
              return <div key={line.key} className="grid gap-3 rounded-lg border border-slate-200 p-3 md:grid-cols-[minmax(10rem,1.5fr)_minmax(7rem,0.7fr)_minmax(8rem,0.8fr)_minmax(8rem,0.7fr)_auto] md:items-end">
                ...
                <Field label="Rate / 1,000"><input type="number" min="0.01" step="0.01" value={line.ratePer1000Bricks} onChange={(event) => updateLine(line.key, "ratePer1000Bricks", event.target.value)} disabled={isSaving} className={inputClass} /></Field>
                <div><p className="text-xs font-medium text-slate-600">Line amount</p><p className="mt-1 flex h-10 items-center font-bold tabular-nums">{amount === null ? "—" : formatSalesMoney(amount)}</p></div>
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-2xl font-bold">Challan #{challan.challanNumber}</h3>
            <StatusBadge status={challan.status} isLocked={challan.isLocked} />
          </div>
      ...
      {challan.status === "void" && <p className="mt-4 rounded-lg border border-slate-300 bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700">This Challan is Void. Its number and historical data remain permanent.</p>}

      {isConfirmingVoid && eligibility.canVoid && <div className="mt-4 rounded-lg border border-red-300 bg-red-50 p-4">
        <p className="text-sm font-bold text-red-900">Void Challan #{challan.challanNumber}?</p>
        <p className="mt-1 text-sm text-red-800">The Challan stays in history and its number will never be reused.</p>
